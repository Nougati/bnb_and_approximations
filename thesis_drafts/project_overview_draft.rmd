...
The field of discrete optimisation \cite{hammer1979discrete} studies problems
 that model discrete decision making. 
...
Optimisation problems with linear constraints and continuous variables are
 ...
 and very efficiently in practice \cite{alizadeh1995interior}.
In this report, we consider maximisation problems whose constraints are all
 linear\cite{alizadeh1995interior}, on the other hand, are characterised by
 having one or more variables constrained to be integer, generally resulting in
 an NP-Hard problem \cite{karp1972reducibility}. 
...
When conducting a Branch and Bound with AAs, an important observation is that
 ...
 standard Branch and Bound with LPs may not apply in our context.
...
Known to be NP-Complete \cite{kellerer2004introduction}, KP has been studied
...
\begin{definition}
A polynomial-time approximation scheme (PTAS) is an algorithm guaranteed to give
...
A classic example of a PTAS was presented by Sahni cite{sahni1975approximate},
 with time complexity $O(n^{1 / \epsilon})$. 
...
\begin{definition}
A fully polynomial time approximation scheme (FPTAS) is an algorithm guaranteed
 to give a solutoin value within a factor of $(1-\epsilon)$ (for maximisation
 problems) of the optimal value, and have a running time bounded by a polynomial
 in the input size $n$ and $\frac{1}{\epsilon}$.
\end{definition}
...
There are nuanced optimisations which supplement this original algorithm's
 approach given by \cite{ibarra1975fast} (for example, the authors define a
 distinction between small profits and large profits), however for simplicity
 we omit such details here.
