...
Implicit enumeration has broad uses within many fields of Computer Science, one
 ...
 partially enumerates through a search space by deriving subproblems and finding
 what are known as their dual bounds on these to guide our search.
...
The set of decision variable values permitted by an optimisation problem's
 constraints is termed the *search space*.
While a problem with allowing continuous variables will have a continuous
 *feasible region* representing its search space, a problem constrained to have
 integer variables will have a *feasible set* of solutions.
...
Optimisation problems with linear constraints and continuous variables are
 ...
 and very efficiently in practice \cite{alizadeh1995interior}. <!--[CLARIFY]Say they have p-time algos then refer to LP solvers research here-->
...
By removing the integer constraints on variables in a MIP, we obtain its
 ...
 program vs linear programming [CLARIFY]-->
...
Branch and Bound commonly solves LP-relaxations to find what are known as
 *primal solutions* that are feasible to the MIP, and find dual bounds to
 implicitly enumerate through these. 
...
This constitutes the core of the Branch and Bound approach, which we formally
 ...
 list of nodes of the Branch and Bound tree yet to be solved (i.e. neither
 pruned nor branched on).
...
While having an exponential worst-case time complexity, the majority of
 computation done in a Branch and Bound is in solving polynomial-time
 LP relaxations which may or may not yield a feasible solution, but always
 provide dual bounds. 
...
However, if a heuristic's solutions has a quality guarantee, in terms of the
 optimal value, it becomes an *Approximation Algorithm* (AA), and we can derive
...
This is a feature not found in LP relaxations, which can provide arbitrarily
 poor dual bounds. 
...
In particular, methods of branching and node selection do not have a clear
 translation in this amended scheme. 
...
Following the inception of Dantzig's  Simplex method
 \cite{dantzig1951maximizatiol} in 1947, theoretical interest in finding optimal
 solutions shifted to harder problems. 
...
Despite its efficiency, LP relaxations can provide arbtirarily poor
 bounds in this context: for Graph Colouring, \cite{coll2002facets} presented a
...
One of the most common examples, performing a DP by values as presented by
 ...
 $P$ is the value of the most profitable item.
...
It is common for FPTAS's for the KP to achieve polynomially bounded run time by
 adjusting the input problem to reduce its complexity, and then use a DP
 algorithm to solve this simplified problem. 
...
Building on the contributions of \cite{ibarra1975fast}, this approach describes
 an alternative FPTAS which still maintains the strategy shown in Algorithm 2. 
...
The authors introduce an auxiliary problem, which they then efficiently devise
 an algorithm for, before using this problem as a generalised formulation of the
 KP. 
...
Central to the approach achieving fast run time is providing a fast solution to
 what is named the *Vector Merging* (VM) problem: for vectors
 $A = (A_1, ... , A_n)$, and $B = (B_0, ... , B_{n-1})$, we want to compute a
 vector $C$ defined by:
...
Because we can evaluate various profit values $q$ of $y_j[q]$ in any order,
 ...
 residual value $r$ from the division $\frac{q}{p_t}$, for each
...
The authors have shown that this is indeed a valid FPTAS which runs in
 $O(n\text{log}(1/3) + 1/\epsilon ^3 \text{log}^2(1/\epsilon))$ time.
...
As well as the original paper in \cite{kellerer2004improved}, the authors also provide an overview of the algorithm in detail in \cite{kellerer2005knapsack}, wher the reader is referred for further information.
...
Given the examples from the literature to Branch and Bound and AAs, our aim is
 now to establish methods for deriving effective dual bounds. 
We will show how we can derive dual bounds from the analysis of the guarantees
 which define a given FPTAS, and present a branching strategy in light of this.
...
In devising a branching strategy, we must consider what variables we can
 constrain which will help us minimise our dual bounds. 
Given that our upper bound on $z_O$ is determined by the scaled values of the
 ...
 presented in section 2.2.1 determines the quality of our dual bound. <!--reword-->

**Insert visualisation of it**

We can attempt to minimise our dual bounds, then, by attempting to maximise the
 difference between $z_{S'}$ and $K\cdot z'_{S'}$, which we can do by first
 branching on variables which truncate the most value in the scaling step. 
...
To improve our approach further, we can specifically design a preprocessing
 scheme intended for finding dual bounds.
An approach to do this is by modifying our value-scaling procedure to round
 all the profits up, instead of rounding down.
