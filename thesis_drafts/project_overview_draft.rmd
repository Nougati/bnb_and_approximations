...
We now give an overview of the range of FPTAS's described that led to the state
 of the art.

### Ibarra and Kim's FPTAS (1975) 
**TODO**: describe the reasoning behind letting $K= \frac{\epsilon P}{n}$ 
...
\begin{proof}
Assume that it is the case. Therefore by Proof By Assumption we have proven it to be true.
\end{proof}
...
The remainder of the description given by \cite{ibarra1975fast}
 extends this core approach by improving the nuance of the preprocessing
 step, which we now describe.

The first improvement results from acknowledging that while a given input
 problem will potentially have many items with very small value, the bulk of the
 value of an optimal solution will likely be from high value items (**is this
 really the actual reason?**). 
We establish a threshold $t$ such that the DP will only compute on items with
 value greater than this. 
This means we can amend our scaling factor in light of the reduced the range of
 item values that our DP considers. 

Within the set of smaller valued items, we conduct a greedy search with the
 remaining space, having completed the algorithm, which is guaranteed to be a
 1/2-approximation (**I think this is right.. but verify this**). 

The chosen value for $t$, in their case, is $\frac{1}{3} \epsilon
 \widetilde{P}$ (**why? maybe it's not necessary to introduce this**).

\begin{lemma}
This is a 1/2-app lol
\end{lemma}
...
Which means that before running the approximation we know *a priori* the range for the optimal value:
