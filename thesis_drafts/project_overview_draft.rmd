...
\emph{I, Nelson Frew, did not plagiaraise this intro segment from Kelvin}
...
Implicit enumeration refers to a divide and conquer problem solving strategy
 where one enumerates through a problem's search space without finding each
 solution. 
ihas broad uses within many fields of Computer Science, one such example
 being in Mixed-Integer Programming's Branch and Bound, where one partially
 enumerates through a search space by deriving subproblems and finding what are
 known as their dual bounds on these to guide our search.
Traditionally, dual bounds are found by solving a relaxed version of the problem
 formulation which can be solved efficiently, however such approaches may perform
 arbitrarily poorly according to problem type.
...
**Disclaimer**
I did this also in the summer of 2017 as a summer Scholar. Also in the latter
 half of 2018 I had heaps of students n junk
...
The field of discrete optimisation \cite{hammer1979discrete} studies problems
 that model discrete decision making, such as scheduling, shortest path problems,
 and supply chain optimisation.
Mixed-Integer Programming refers to a set of modelling and solving techniques for
 such discrete problems \cite{wolsey2007mixed} by formulating them as
 Mixed-Integer Programs (MIPs).  <!-- Swapping between formulating and modelling-->
For many problems, this is the most efficient known method to find a solution. 
...
<!--Arthur said something about doing stuff in a different order in this section [CLARIFY]-->

<!--TODO: Put a paragraph here about linear programming relaxations-->
<!--TODO: Put in a paragraph talking about feasible regions-->
...
Optimisation problems with linear constraints and continuous variables are
 ...
 and very efficiently in practice \cite{alizadeh1995interior}. <!--Say they have p-time algos then refer to LP solvers research here-->
In this report, we consider maximisation problems whose constraints are all
 linear\cite{alizadeh1995interior}. 
...
By removing the integer constraints on variables in a MIP, we obtain its
 associated *LP relaxation*.<!--Make it clear when it is to be read at linear
 program vs linear programming -->
...
<!-- Redo this so that paragraphs compartmentalise branching then bounding -->
...
The state-of-the-art method for solving MIPs uses this relationship to find 
 what are known as *primal solutions* that are feasible to the MIP, as well as
 dual bounds to implicitly enumerate through these. 
...
If we find that a given subproblem's dual bound is lower than the MIP's primal
 ...
 and constraining of this subproblem cannot yield a better primal solution.<!--Arthur: yes but not w/o the relax relationship [CLARIFY]-->
...
This constitutes the core of the Branch and Bound approach, which we formally
 ...
 solution to the MIP, $\ubar z$ the lower bound on the optimal value
 , $z^*$ the optimal solution of the MIP, and let $\mathcal{L}$ denote the
...
<!-- TODO: break in narrative, via heuristics. This would mean motivating use of
     dual bounds earlier (Pierre was like "oh yeah there are these heuristics, 
     but they don't serve our purposes  -->
...
While finding optimal solutions with Branch and Bound is in worst case
 ...
 which can provide feasible solutions in as fast as polynomial-time. <!--Reword-->
...
This is a feature not found in LP relaxations, as they can provide arbitrarily
 poor dual bounds. 
...
When conducting a Branch and Bound with AAs, an important observation is that
 ...
 standard Branch and Bound with LPs may not apply in our context. <!--Sentence too long-->
...
Following the inception of Dantzig's \cite{dantzig1951maximization} Simplex
 method in 1947, theoretical interest in finding optimal solutions shifted to
 harder problems. 
...
The definition of "quality" is clearly important:  within a Branch and Bound
 a basic but useful indicator could be any information about a node's dual bound. **HELP**
...
Despite its efficiency, LP relaxations can provide arbtirarily poor
 ...
 graph will be generally far from optimal (CITE?). <!--TODO: put other case study here-->
...
The cutting plane approach, introduced by \cite{dantzig1954solution}, and
 ...
 called a cutting plane, that separates this solution from the rest of the
 search space. 
...
Known to be NP-Complete \cite{kellerer2004introduction}, the KP has been studied
 ...
 it serves well as a topic for analysis. 
...
It is well known that the KP lends itself well to a Dynamic Programming (DP)
 approach, although there are many ways to conduct this. <!--Citation?-->
One of the most well known examples, performing a DP by values as presented by
 ...
 $P$ is the value of the most profitable item which we now describe: let
 $A(i, p)$ be the minimal weight of the solution to KP with only the first $i$
 items available, where the total value is exactly $p$. 
...
Being NP-Hard, the KP does not admit a polynomial time algorithm, however we can
...
A Polynomial-Time Approximation Scheme is an algorithm guaranteed to give
 ...
 input size $n$, for fixed values of $\epsilon$.
\end{definition}
...
While the run time of a PTAS is indeed bounded by a polynomial in $n$, the
 complexity can be exponential in terms of $\epsilon$, so long as $\epsilon$ is
 fixed. 
...
A Fully Polynomial Time Approximation Scheme is an algorithm guaranteed
 to give a solutoin value within a factor of $(1-\epsilon)$ (for maximisation
 problems) of the optimal value, and to have a running time bounded by a
 polynomial in the input size $n$ and $\frac{1}{\epsilon}$.
\end{definition}

Commonly, FPTAS's for the KP achieve polynomially bounded run time by adjusting
 the input problem to reduce its complexity, then using a DP algorithm to solve
...
We consider the examples from the literature which have led to what are now
 textbook examples of FPTAS's to show the performance of well established
 approximations when used in a Branch and Bound scheme, followed by a
 description of the state-of-the-art method.
...
The distillation of their algorithm, as described by <!--Wordy-->
...
\begin{lemma}
For the item set returned by the approximation, $S'$, its associated approximated value $z_{S'}$, and value of the optimal solution set $z_O$, the following inequality holds:
...
Since $K = \frac{\epsilon P}{K}$, and $P \leq z_O$:
...
For simplicity, we omit further refinements to this algorithm which accompanies
...
For some integer $j \leq n$, let $A_j$ be an array which contains
 representations of solutions to KP for the first $j$ items.
...
### Kellerer and Pferschy (2004) *UNFINISHED*
The final example of and FPTAS for KP which we will review was presented by
...
The authors introduce what they refer to as a *vector merging problem*, which
 they then efficiently devise an algorithm for, before using this problem as a
 generalised formulation of the KP. 

The preprocessing step proposed involves, just as in previous FPTAS's, making
 a distinction between items with "large" and "small" profits.
All items with large profits are then organised into equal length intervals of
 ...
 with their respective value ranges.
Then, a subset of each given subinterval's smallest weight items are kept,
 according to the given subinterval and the error parameter $\epsilon$.
These chosen items then have all their profits reduced to the lower bound of
...
\begin{proof}
**TODO PROVE ME** (also make sure that I have introduced everything to make sure
...
Central to the approach achieving fast run time is providing a fast solution to
 what is named the *Vector Merging Problem*: for vectors $A = (A_1, ... , A_n)$,
 and $B = (B_0, ... , B_{n-1})$, we want to compute a vector $C$ defined by:

\DeclarePairedDelimiter\set\{\}

$$C(k) := \text{min}\set[\Bigg]{A(l) = \sum_{j=0}^{k-l} B(j) \text{ }\Bigg|\text{ } l = 1, \dots , k}, k = 1, \dots, n$$

While this problem can be trivially solved in $O(n^2)$ time, we can solve this
 problem fully in $O(n\text{log}n)$ time. 
The basis for this algorithm is as follows: **TODO**

The DP scheme that we employ maintains an array $y_j[q]$, which has the minimum
 weight solution for the first $j$ items with profit $q$. 
Because we can evaluate various profit values $q$ of $y_j[q]$ in any order,
 we simultaneously solve ... **TODO residual value explanation**

**TODO: Simultaneous profit evaluation explanation and how it looks like Vmerge**

**TODO: Tie it up into an interval DP**

**TODO: Talk about some semantics about finding item sets and ties it all up**

## Approximations, Linear Programming and Branch and Bound 

**Note: I haven't amended this to remove linear duality up to this point**
...
The result of this produced the only implicit enumeration scheme leveraging AA
 guarantees that we are aware of.
...
**TODO: Something something something truncation branching (does this need its own section?) Where should it go?**
...
## Derivation of dual bounds *a posteriori* UNFINISHED
...
## Improved a posteriori dual bounds **UNFINISHED**
...
As such, further work into experimentation with more sophisticated FPTAS's may
 have merit. 
...
It will, of course, be worth investing time in investigating the viability of
 ...
 Christofides' \cite{christofides1976worst} approximation for the Metric
